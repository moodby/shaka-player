...
shaka.media.StreamingEngine = class {
  ...
  async start() {
    window.postMessage(JSON.stringify({
      ...
      'traceActivity': 'shaka.media.StreamingEngine.start',
    }));
    goog.asserts.assert(this.config_,
        'StreamingEngine configure() must be called before init()!');
    window.postMessage(JSON.stringify({
      'type': 'MOODBY_TRACE_EVENT',
      'traceActivity': 'shaka.media.StreamingEngine.startInitStreams_',
    }));
    // Setup the initial set of Streams and then begin each update cycle.
    await this.initStreams_();
    window.postMessage(JSON.stringify({
      'type': 'MOODBY_TRACE_EVENT',
      'traceActivity': 'shaka.media.StreamingEngine.startInitStreams_Resolved',
    }));
    ...
    window.postMessage(JSON.stringify({
      'type': 'MOODBY_TRACE_EVENT',
      'traceActivity': 'shaka.media.StreamingEngine.startStartupComplete_',
    }));
  }
  ...
  async fetch_(mediaState, reference, streamDataCallback, isInit) {
    window.postMessage(JSON.stringify({
      'type': 'MOODBY_TRACE_EVENT',
      'traceActivity': 'shaka.media.StreamingEngine.fetch_',
      'referenceStartByte': reference.startByte,
      'referenceEndByte': reference.endByte,
      'isInit': isInit,
    ...
    if (reference instanceof shaka.media.InitSegmentReference) {
      ...
      if (segmentData) {
        window.postMessage(JSON.stringify({
          'type': 'MOODBY_TRACE_EVENT',
          'traceActivity':
            'shaka.media.StreamingEngine.fetch_ReturnSegmentData',
          'referenceStartByte': reference.startByte,
          'referenceEndByte': reference.endByte,
          'isInit': isInit,
          'uris': reference.getUris(),
    ...
    if (
      ...
      window.postMessage(JSON.stringify({
        'type': 'MOODBY_TRACE_EVENT',
        'traceActivity':
          'shaka.media.StreamingEngine.fetch_getPrefetchedSegment',
        'referenceStartByte': reference.startByte,
        'referenceEndByte': reference.endByte,
        'isInit': isInit,
        'uris': reference.getUris(),
    ...
    if (!op) {
      window.postMessage(JSON.stringify({
        'type': 'MOODBY_TRACE_EVENT',
        'traceActivity':
          'shaka.media.StreamingEngine.fetch_dispatchFetch_',
        'referenceStartByte': reference.startByte,
        'referenceEndByte': reference.endByte,
        'isInit': isInit,
        'uris': reference.getUris(),
    ...
    window.postMessage(JSON.stringify({
      'type': 'MOODBY_TRACE_EVENT',
      'traceActivity':
        'shaka.media.StreamingEngine.fetch_fetchOperation',
      'referenceStartByte': reference.startByte,
      'referenceEndByte': reference.endByte,
      'isInit': isInit,
      'uris': reference.getUris(),
    ...
    window.postMessage(JSON.stringify({
      'type': 'MOODBY_TRACE_EVENT',
      'traceActivity':
        'shaka.media.StreamingEngine.fetch_fetchOperationResolved',
      'referenceStartByte': reference.startByte,
      'referenceEndByte': reference.endByte,
      'isInit': isInit,
      'uris': reference.getUris(),
  ...
  dispatchFetch_(reference, stream, streamDataCallback, isInit) {
    ...
    window.postMessage(JSON.stringify({
      'type': 'MOODBY_TRACE_EVENT',
      'traceActivity':
        'shaka.media.StreamingEngine.dispatchFetch_',
      'segmentType': type,
    }));
