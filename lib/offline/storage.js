...
shaka.offline.Storage = class {
  ...
  async remove_(contentUri) {
    ...
    try {
      ...
      // Correct remove order with catching..
      // Order is simportant. We should remove from storage first.
      await this.removeFromStorage_(cell, uri, manifest);
      // We do not want to throw error, when failed to remove drm license..
      // Ignore this error, expecting license will expired.
      // Its enough for us, that track was removed from storage.
      await this.removeFromDRM_(uri, manifest, muxer).catch(console.warn);
    } finally {
      await muxer.destroy();
    }
  }
  ...
  async removeEmeSessions_() {
    ...
    try {
      ...
      for (const sessionIdCell of cells) {
        ...
        // * Moodby
        // const deletedSessionIds = await deleter.delete(
        //  config, net, sessions);
        ...
        for (const session of sessions) {
          const deletedSessionIds = await deleter.delete(config, net, [
            session,
          ]);
          await sessionIdCell.remove(deletedSessionIds);
